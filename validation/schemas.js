...
module.exports = {
  ...
  purchaseOrder: {
    poNumber: str(true), supplier: { ...id, required: true }, supplierName: str(), category: str(), orderDate: date, expectedDate: date,
  ...
  goodReceipt: {
    grnNumber: str(true), poNumber: str(), supplier: id, supplierName: str(), category: str(), receiptDate: date,
    itemsCount: num(false), quantity: num(false), unit: str(), receivedBy: str(), receiptValue: num(false), items: { type: 'array', of: grnItem },
    status: { type: 'string', enum: status.grn },
  },
  ...
  stockTransaction: { itemType: { type: 'string', required: true, enum: status.itemType }, stockId: { ...id, required: true }, quantity: num(true, 1), remarks: str(), reason: str(), direction: { type: 'string', enum: status.direction } },
