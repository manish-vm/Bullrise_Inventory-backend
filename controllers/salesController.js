...
exports.updateStatus = asyncHandler(async (req, res) => {
  ...
  if (nextStatus === 'Confirmed' && order.status === 'Pending') {
    for (const item of order.items) {
      const stock = await FinishedGoodsStock.findOne({ sku: item.sku }).sort('-availableQuantity');
  ...
  if (nextStatus === 'Packed' && order.status === 'Confirmed') {
    await Activity.create({ module: 'sales', title: `${order.orderNo} packed`, description: 'Reserved stock moved to packing', dateText: new Date().toLocaleString(), type: 'purple' });
  }

  if (['Shipped', 'Delivered'].includes(nextStatus) && ['Confirmed', 'Packed'].includes(order.status)) {
    for (const item of order.items) {
      const stock = await FinishedGoodsStock.findOne({ sku: item.sku }).sort('-reservedQuantity');
  ...
  await order.save();
  await Activity.create({ module: 'sales', title: `${order.orderNo} ${nextStatus.toLowerCase()}`, description: `Sales order status changed to ${nextStatus}`, dateText: new Date().toLocaleString(), type: nextStatus === 'Cancelled' ? 'danger' : 'success' });
  ok(res, order);
});
